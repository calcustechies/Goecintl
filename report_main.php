<?php
	...
	{
		$formula6="AND `nationality`='$customer_country'";
		$formula= $formula." ".$formula6;
	}

	//if ($gain != '0')
	//{
		//$formula5="AND `us_code`='$user'";
...
function Footer()
{
	...
	$pdf->Line(2, 35, 400,35);
		...
		if($customer == '0')
		{
			$pdf->Cell(55,8,"Cust. Name",1,0,'L');
			$pdf->Cell(25,8,"Telephone",1,0,'C');
			$pdf->Cell(35,8,"ID/PP No",1,0,'C');
		}else
		{
			$pdf->Cell(115,8,"Narration",1,0,'C');
		}
		...
		while($rows = mysqli_fetch_assoc($resultset)) {
			...
			if ($customer == '0') {
				$pdf->Cell(55,6,substr($rows['cus_name'],0,25),1,0,'L');
			...
			//$pdf->Cell(10,6,$rows['sale_pur'],1,0,'L');
			...
			//purchase and loss negative value
			...
			if (strpos($rows['to_amt'], ".") != false) {
				...
				//----------------------------------------------------
							...
							//$pdf->Cell(20,6,$rows['to_amt'],1,0,'R');
				...
				if($trans_type != 2)
				{
					if($trans_type  == 1)
					{
						...
						$pdf->Cell(10,6,$rowstt['cy_name'],1,0,'R');
					}else {
			...
			$pdf->Cell(15,6,round($rows['cmsn'],3),1,0,'R');
		...
		if($currency == '0')
		{
			$pdf->Cell(231,6,"Total",1,0,'L');
			
		}else
		{
			$pdf->Cell(172,6,"Total",1,0,'L');
			//----------------------------------------------------
			if (strpos($tot_from_amt, ".") != false) {
				...
				//----------------------------------------------------
			//$pdf->Cell(39,6,$tot_from_amt,1,0,'R');
		}
		//----------------------------------------------------
		if($currency == '0')
		{
		...
		}else
		{
			...
			$tmlen=strlen($tm2);
				if($tmlen < '3')
				{
					if($tmlen == '1')
					{
						$pdf->Cell(40,6,$tot_to_amt."00",1,0,'R');
					}elseif ($tmlen == '2') {
						$pdf->Cell(40,6,$tot_to_amt."0",1,0,'R');
					}
				}else {
					$subtm2=substr($tm2,0,3);
					$pdf->Cell(40,6,$tm1.".".$subtm2,1,0,'R');
				}
			}
			else{
						$pdf->Cell(40,6,$tot_to_amt.".000",1,0,'R');
			}
		}
			//----------------------------------------------------

		//$pdf->Cell(20,6,$tot_to_amt,1,0,'R');
		//$pdf->Cell(10,6,"",1,0,'R');
		$pdf->Cell(25,6,round($cmsn,3),1,0,'R');
